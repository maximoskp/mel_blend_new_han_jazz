...
import NN_network_evaluation as ne
import os
cwd = os.getcwd()
import random
...
class Melody:
    ...
    def __init__(self, s):
        ...
        self.fitness = []
    # end constructor
    def compute_feature_markov_NN_fitness(self, f, m):
        ...
        v3 = max( netEval.eval_nn_stream_mel( self.stream ) , 0)
        self.fitness = 0.8*v1 + 0.1*v2 + 0.1*v3
    # end compute_feature_markov_NN_fitness
...
class EvoSession:
    ...
    def __init__(self, file1_name, file2_name, target_features, target_markov, nPop=300, nGen=200, print_gens=True):
        ...
        self.parent1.compute_feature_markov_NN_fitness( target_features, target_markov )
        self.parent2.compute_feature_markov_NN_fitness( target_features, target_markov )
    ...
    def initialise_random_piece_from_folders( self, folders_in, extension='.xml' ):
        ...
        if len( desired_file_names ) == 0:
            ...
            # compute fitness of parents
            out_individual.compute_feature_markov_NN_fitness( self.target_features, self.target_markov )
    ...
    def evolution_round(self):
        ...
        while len(self.nextPop) < self.nPop:
            ...
            m1 = Melody(c1)
            m1.compute_feature_markov_NN_fitness( self.target_features, self.target_markov )
            m2 = Melody(c2)
            m2.compute_feature_markov_NN_fitness( self.target_features, self.target_markov )
