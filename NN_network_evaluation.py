...
import MBL_melody_features_functions as mf
import NN_piece_info as piece_info
...
class NetEval:
    ...
    def eval_feats_nn_stream_mel(self, s, target_features):
        ''' s is a music21 stream '''
        # print('eval_feats_nn_stream_mel')
        # get piece_info structure from stream
        f = mf.get_features_of_stream( s )
        nn_eval = max( self.eval_nn_stream_mel( s ) , 0)
        f = np.append( f, nn_eval )
        target_features = np.append( target_features, 0.0 )
        nn_contrib = 1.0/len( f + 1 )
        d = f - target_features
        d[0:len( f )] = (1-nn_contrib)*d[0:len( f )]
        d[ len( f ) ] = nn_contrib*d[ len( f ) ]
        ...
        '''
        return n
    # end eval_nn_stream_mel

    def eval_nn_stream_mel(self, s):
        ''' s is a music21 stream '''
        # print('eval_nn_stream_mel')
        # get piece_info structure from stream
        p_info = piece_info.PieceClass( s )
        seed = self.initial_seed
        tmpSum = 0
        for i in range( p_info.binary_matrix.shape[1] ):
            if i > 0:
            ...
            predicted_distr = predicted_distr/np.sum(predicted_distr)
            melody_output = p_info.binary_matrix[:,i]
            ...
            if kl > 15:
                kl = 100
            tmpSum += kl
        return tmpSum/p_info.binary_matrix.shape[1]
    # end eval_nn_stream_mel
